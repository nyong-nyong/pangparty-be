...
public interface RollingPaperStickerRepository extends JpaRepository<RollingPaperSticker, Long> {
    ...
    @Query("select s from RollingPaperSticker s left join fetch s.sticker where s.rollingPaper.uid = ?1 order by s.topLoc asc")
    List<RollingPaperSticker> findRollingPaperStickersByRollingPaperUid(Long rollingPaperUid);
